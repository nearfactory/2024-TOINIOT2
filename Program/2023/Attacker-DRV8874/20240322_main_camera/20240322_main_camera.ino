...
// カメラの視野幅は320px（左右から100pxずつの位置にそれぞれ閾値ラインを設ける）
#define LEFT_LIM 100
#define RIGHT_LIM 220
...
void setup() {
  ...
  while(true){
    ...
    if(digitalRead(btnPin[0]) == 1){
      waitBtn(0);
      opponentColor = cBtnCount % 2 + 1;  // 1: 青 2: 黄
...
void loop() {
  ...
  refreshCamera(pixy);
  if(blockNum != 0 && blockX < LEFT_LIM && blockSignature == opponentColor){
    lastGoal = 1;
  }
  else if(blockNum != 0 && blockX > RIGHT_LIM && blockSignature == opponentColor){
    lastGoal = 2;
  }
  else if(blockNum != 0 && blockSignature == opponentColor){
    lastGoal = 0;
  }
}